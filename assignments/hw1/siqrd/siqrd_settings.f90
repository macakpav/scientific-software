...
! editable settings
  ...
  integer, parameter        :: NO_STEPS      = 150                       ! number of steps
  real(wp), parameter       :: TIME_HORIZON  = 30.                       ! final time
...
! Newton's method settings (only applies for backward Euler)
  ...
  real(wp), parameter       :: TOLERANCE     = 10.**(-precision(0._wp)+1) ! tolerance in Newton's method
  logical, parameter        :: ABORT_CONVERGENCE = .true.                 ! should the simulation abort if desired TOLERANCE wasn't reached in MAX_ITER iterations
