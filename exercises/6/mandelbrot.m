...
C = real(center) - radius + 2 * (X - 1) * radius / (steps - 1) ...
  +1i * (imag(center) - radius + 2 * (Y - 1) * radius / (steps - 1));
R_tilde = maxiter * ones(steps);

for n = 1:steps
  for m = 1:steps
    c=C(m,n); %this is faster than evaluating function for c on every iteration
    z=c;
    for r = 1:maxiter
      z = z^2 + c;
      if abs(z) > 2
