...
namespace tws
{
    ...
    template <typename S1, typename V2>
    typename std::enable_if<std::is_arithmetic<S1>::value && is_vector<V2>::value, scalar_vector_multiply<S1, V2>>::type operator*(S1 const &s1, V2 const &v2)
    {
        return scalar_vector_multiply<S1, V2>(s1, v2);
    } //operator*

    template <typename S1, typename V2>
    typename std::enable_if<std::is_arithmetic<S1>::value && is_vector<V2>::value, scalar_vector_multiply<S1, V2>>::type operator*(V2 const &v2, S1 const &s1)
    {
        return scalar_vector_multiply<S1, V2>(s1, v2);
    } //operator*
