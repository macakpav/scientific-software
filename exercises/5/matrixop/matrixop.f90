...
contains
    ...
    subroutine mm_blocks_a(A, B, C, blocksize)
        ...
        integer, target :: first,second,third
        integer noBlocks,firstB,secondB,thirdB
        j => first
        k => second
        i => third
        C = 0.0_dp
        noBlocks=size(A,1)/blocksize
        do firstB = 0,noBlocks-1
            do secondB = 0,noBlocks-1
                do thirdB = 0,noBlocks-1
                    do first = 1+firstB*blocksize,(firstB+1)*blocksize
                        do second = 1+secondB*blocksize,(secondB+1)*blocksize
                            do third = 1+thirdB*blocksize,(thirdB+1)*blocksize
                                C(i,j) = C(i,j) + A(i,k)*B(k,j)
                            enddo
                        enddo
                    enddo
