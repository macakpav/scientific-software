...
extern "C"
{
    ...
    void fortran_eig( void (*funptr)(double A[],int & n,double x[], double y[]), double *matrix_values, int &matrix_size, double *eigvr, double *eigvi) ;
    void avc(double A[], int & matrix_size, double x[], double y[]) ;
}
int cpp_eig(double *matrix_values, int matrix_size, double *eigvr, double *eigvi){
    
    fortran_eig(avc, matrix_values, matrix_size, eigvr, eigvi) ;
